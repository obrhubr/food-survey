...
export default function Menu() {
    ...
    return (
		<>
            <div className='text-2xl text-center m-4 font-bold bg-gray-200 rounded-md p-4'>
                RANKING
            </div>
                            ...
                            {ranking.sort((f, s) => { if(strat.inverted ^ order) { return s.data[strat.name].total - f.data[strat.name].total } else { return f.data[strat.name].total - s.data[strat.name].total } }).map((item, i) => {
                                return(
                                    <div key={i} className='bg-gray-200 rounded-md p-2 m-2 flex flex-row'>
                                        <div className='flex-grow w-2/6'>{i+1}. <a className='underline' rel="noreferrer">{item.name}</a></div>
                                        {strat.show_scale ?
                                        ...
                                        <div className='w-1/6 px-6 text-gray-400'>Score: {Math.round(item.data[strat.name].total * 10) / 10}</div>
                                    </div>
                                )
                            })}
                        </div>
